...
module Plushie
  ...
  module UI
    ...
    #
    # Supports rich composition with table_row/cell children or data shorthand
    # via the :rows prop (expanded to children during normalization).
    #
    # @param id [String] widget ID
    # @param props [Hash] table options (:columns, :rows, :on_sort, :on_select, etc.)
    # @yield column definitions and row content
    # @return [Node]
    # @example Rich composition
    ...
    #
    # @param id [String] row ID (used for selection and diffing)
    # @param props [Hash] row options
    # @yield cell children
    # @return [Node]
    ...
    #
    # @param id [String] column key this cell belongs to
    # @param content_or_props [Object, Hash] either a single child node or props
    # @param props [Hash] cell options (:column, etc.)
