...
2. **Rust side:** implement the `WidgetExtension` trait from `plushie-widget-sdk`. This
...
The `plushie_widget_sdk::prelude` re-exports `alignment`, `Point`, and `Size`,
so you do not need to reach into `plushie_widget_sdk::iced::alignment` for
...
`rake plushie:build` shells out to
[`cargo-plushie`](https://crates.io/crates/cargo-plushie) for the
heavy lifting. That tool is shared across every Plushie host SDK so
workspace generation, patch forwarding, and widget discovery live in
one place.

### How it works

1. **Resolves widgets.** Reads the configure block first, then
   falls back to `PLUSHIE_WIDGETS` (or `PLUSHIE_EXTENSIONS`). Each
   class is validated to be a `native_widget`.
2. **Validates crate paths.** Each widget's `rust_crate` path is
   resolved against the project root. Traversal outside the project
   root is rejected.
3. **Verifies widget metadata.** Each crate's `Cargo.toml` must
   declare a `[package.metadata.plushie.widget]` table with
   `type_name` and `constructor` keys. cargo-plushie uses those to
   discover widgets during `cargo metadata`.
4. **Writes a virtual app manifest** under `_build/plushie-renderer-spec/`
   listing the widget crates as path dependencies. This exists only
   so `cargo-plushie` has a dep graph to walk.
5. **Invokes `cargo plushie build`.** The tool generates the real
   renderer workspace under
   `_build/plushie-renderer-spec/target/plushie-renderer/`, wires in
   every discovered widget, and runs `cargo build`.
6. **Installs the binary** into `_build/plushie/bin/` where
   `Plushie::Binary.resolve` finds it.

### Resolving cargo-plushie

The SDK looks for `cargo-plushie` in this order:

1. If `PLUSHIE_RUST_SOURCE_PATH` points at a plushie-rust checkout,
   run the workspace copy via `cargo run -p cargo-plushie`. No
   install needed; the tool tracks the checkout.
2. Otherwise, use `cargo-plushie` on `PATH` if its `--version`
   matches `Plushie::PLUSHIE_RUST_VERSION`.
3. Otherwise, fail with guidance:
   `cargo install cargo-plushie --version <PLUSHIE_RUST_VERSION> --locked`.

The widget-crate Cargo.toml needs the metadata table:
