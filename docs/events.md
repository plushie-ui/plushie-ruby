...
<!-- test: events_widget_scroll_data_match -- keep this code block in sync with the test -->
```ruby
...
<!-- test: events_widget_paste_match -- keep this code block in sync with the test -->
```ruby
...
<!-- test: events_widget_open_match, events_widget_close_match -- keep this code block in sync with the test -->
```ruby
...
<!-- test: events_widget_sort_match -- keep this code block in sync with the test -->
```ruby
...
Mouse area events are delivered as `Event::Widget` with prefixed types:

<!-- test: events_mouse_area_enter_match, events_mouse_area_move_match -- keep this code block in sync with the test -->
```ruby
Event::Widget[type: :mouse_right_press, id: "canvas"]
Event::Widget[type: :mouse_enter, id: "tooltip-target"]
Event::Widget[type: :mouse_move, id: "drag-zone", data: {x:, y:}]
Event::Widget[type: :mouse_scroll, id: "scroll-zone", data: {delta_x:, delta_y:}]
```
...
in Event::Widget[type: :mouse_move, id: "canvas_area", data: {x:, y:}]
...
the canvas node. Canvas events are delivered as `Event::Widget`.

<!-- test: events_canvas_press_match, events_canvas_move_match -- keep this code block in sync with the test -->
```ruby
Event::Widget[type: :canvas_press, id: "draw_area", data: {x:, y:, button: :left}]
Event::Widget[type: :canvas_release, id: "draw_area", data: {x:, y:, button: :left}]
Event::Widget[type: :canvas_move, id: "draw_area", data: {x:, y:}]
Event::Widget[type: :canvas_scroll, id: "draw_area", data: {x:, y:, delta_x:, delta_y:}]
```

The `button` field is a symbol (`:left`, `:right`, `:middle`). The
`x`/`y` coordinates are relative to the canvas origin.

```ruby
in Event::Widget[type: :canvas_press, id: "draw_area", data: {x:, y:, button: :left}]
  model.with(drawing: true, last_point: [x, y])

in Event::Widget[type: :canvas_move, id: "draw_area", data: {x:, y:}]
  if model.drawing
...
Generated by the renderer when it detects a configuration issue with a
canvas element (e.g. missing accessibility metadata). Useful for
...
<!-- test: events_sensor_resize_match -- keep this code block in sync with the test -->
```ruby
Event::Widget[type: :sensor_resize, id: "content_area", data: {width:, height:}]
```

```ruby
in Event::Widget[type: :sensor_resize, id: "content_area", data: {width:, height:}]
...
<!-- test: events_pane_resized_match, events_pane_clicked_match -- keep this code block in sync with the test -->
```ruby
Event::Widget[type: :pane_resized, id: "editor", data: {split:, ratio:}]
Event::Widget[type: :pane_dragged, id: "editor", data: {pane:, target:}]
Event::Widget[type: :pane_clicked, id: "editor", data: {pane:}]
```

```ruby
in Event::Widget[type: :pane_clicked, id: "editor", data: {pane:}]
...
<!-- test: events_key_release_match -- keep this code block in sync with the test -->
```ruby
...
<!-- test: events_ime_preedit_match, events_ime_commit_match -- keep this code block in sync with the test -->
```ruby
...
<!-- test: events_mouse_cursor_moved_match, events_mouse_button_pressed_match, events_mouse_wheel_scrolled_match -- keep this code block in sync with the test -->
```ruby
...
<!-- test: events_touch_finger_pressed_match -- keep this code block in sync with the test -->
```ruby
...
<!-- test: events_modifiers_changed_match -- keep this code block in sync with the test -->
```ruby
...
<!-- test: events_window_close_requested_match, events_window_resized_match, events_window_file_dropped_match -- keep this code block in sync with the test -->
```ruby
...
<!-- test: events_system_animation_frame_match, events_system_theme_changed_match -- keep this code block in sync with the test -->
```ruby
