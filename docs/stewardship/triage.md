...
2. **Elm invariants.** Does the work touch the
   `init`/`update`/`view` contract, return-shape validation,
...
4. **Renderer-to-host integrity.** Does the work touch the
   ...
   (passing through unknown event variants, opaque-blob
   delivery to user code, spoofable response correlation, an
   unsafe parser, calling `to_sym` on untrusted strings on a
   hot path)? Treat as a deliberate decision, not a routine
...
5. **Resilience axis.** Does the work address a real things-go-
   ...
   the runtime, a missed revert on view error, a stale effect
   tag, a bridge that hangs instead of triggering restart, a
   `rescue Exception` that swallows interrupts), or
...
6. **Wire codec correctness.** Encode/decode symmetry, round-
   trip through MessagePack and JSONL, field-name drift between
   encoder and renderer, fail-fast on unknown types in
   `Encode.encode_value`. Do; stated goal.
...
7. **Lightweight by default.** Does the work consolidate
   ...
   (an extra allocation, redundant key stringification, an
   `each` loop that another method already did), while
   preserving or improving readability? Do; reference
   `performance-bar.md`. Conversely, is the work clever-for-
   speed at the cost of intent, or a big-O claim without
   realistic N? Decline absent measurement.

8. **Test discipline.** Does the work move tests off the
   integration spine (mocking the renderer, replacing real
   binary tests with pure-Ruby stubs, reaching into runtime
   instance variables instead of using the session API)?
   Decline; reference `test-discipline.md`. Does it move tests
   onto the spine (rewriting a stub test to run through the
   binary)? Do.

9. **DSL extension.** Does the proposal add a new declaration
   form, a new block scope, a new event spec form, a new
   metaprogramming pattern? Run the criteria in
   `dsl-discipline.md` (two real users, real bug class,
   generated code reads cleanly, error messages point at the
   call site, `instance_eval`/`method_missing` justified by a
   real win that ordinary code cannot deliver). If it does
   not pass, decline or defer.

10. **Concurrency shape change.** Does the work introduce a new
    long-lived thread, change the runtime/bridge thread
    contract, switch the runtime to fibers or ractors, or move
    work between Connection, Bridge, and Runtime? Treat as a
    stewardship-level question; reference
    `concurrency-shape.md`.

11. **Simplicity axis.** Single-user mixin extracted as a
    reusable module? Module split without a forcing function?
    Premature generic where specific would do? Decline;
    reference `simplicity.md`. Conversely, three-similar-lines
    that have grown into a real concept and want to be
    abstracted? Do.
