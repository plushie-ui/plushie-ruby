...
class TestCommand < Minitest::Test
  ...
  def test_move_cursor_to
    ...
    assert_equal(5, cmd.payload[:value])
  end
  ...
  def test_select_range
    ...
    assert_equal({ start_pos: 2, end_pos: 8 }, cmd.payload[:value])
  end
  ...
  def test_advance_frame
    cmd = C.advance_frame(16_000)
    assert_equal :advance_frame, cmd.type
    assert_equal 16_000, cmd.payload[:timestamp]
  end
end
